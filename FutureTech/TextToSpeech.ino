...
void loop() {
  if (numRuns <= maxRuns) {
    Serial.println("Running TextToSpeech - Run #" + String(numRuns++));

    TembooChoreo TextToSpeechChoreo(client);

    // Invoke the Temboo client
    TextToSpeechChoreo.begin();
    ...
    while(TextToSpeechChoreo.available()) {
      char c = TextToSpeechChoreo.read();
      Serial.print(c);
    }
  ...
  Serial.println("\nWaiting...\n");
  delay(30000); // wait 30 seconds between TextToSpeech calls
}